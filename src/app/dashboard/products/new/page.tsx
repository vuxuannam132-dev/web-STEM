...
export default function NewProductPage() {
        ...
        <div key={i} className="flex gap-2">
          ...
          {list.length > 1 && (
            <button
              ...
              className="p-2 rounded-xl hover:bg-red-50 text-red-500 transition-all animate-fade-in"
            >
              <X className="w-4 h-4" />
            </button>
          )}
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Đăng Sản Phẩm Mới</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <GlassCard className="space-y-5">
          ...
          <GlassInput
            label="Tên sản phẩm"
            placeholder="VD: Robot dò đường tự động"
            value={title}
          ...
          {slug && (
            <p className="text-slate-500 text-xs">
              Slug: <code className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded">{slug}</code>
            </p>
          )}

          <GlassTextarea
            label="Mô tả sản phẩm"
            placeholder="Mô tả chi tiết về sản phẩm STEM..."
            value={description}
        ...
        <GlassCard className="space-y-5">
          ...
          {renderListField('Người thực hiện', authors, setAuthors, 'Họ tên người thực hiện')}
        ...
        <GlassCard className="space-y-5">
          ...
          {renderListField('Kiến thức áp dụng', knowledge, setKnowledge, 'VD: Điện tử cơ bản')}
          {renderListField('Thành phần cấu tạo', components, setComponents, 'VD: Arduino Uno')}
        </GlassCard>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-950 placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all"
              />
              <button
                type="button"
                onClick={addTag}
                className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all text-sm font-medium"
              >
                Thêm
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
              {tags.map((tag, i) => (
                <span key={i} className="flex items-center gap-1 px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-xs">
                  #{tag}
                  <button type="button" onClick={() => setTags(tags.filter((_, j) => j !== i))} className="hover:text-red-500 transition-colors">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
        ...
        <GlassCard className="space-y-5">
          <h2 className="text-lg font-semibold text-slate-800">Tùy chỉnh QR Code</h2>
          ...
          <GlassInput
            label="Text dưới QR (tùy chọn)"
            placeholder="VD: Quét để xem chi tiết"
            value={qrFrameText}
